...
The simplest phylogenetic clock model -- the strick clock -- makes the mathematically convenient assumption that the evolutionary rate is constant across all lineages \cite{zuckerkandl1965evolutionary, kuhner1995estimating, larget1999markov}.  % Early examples? 
...
Adaptive operators undergo a training process to improve their efficiency over time \cite{roberts2007coupling}. 
For example, the conditional clade probabilities of neighbouring trees can serve as the basis of adaptive tree operators \cite{hohna2012guided,zhang2020using,meyer2019adaptive},
and the mirror kernel learns a target distribution which acts as a `mirror image' of the current point $x$ \cite{thawornwattana2018designing}.  
The \texttt{AdaptiveOperatorSampler} operator, introduced in this article, learns the weight of sub-operators.
Finally, the adaptable variance multivariate normal (AVMVN) kernel  learns correlations between parameters during MCMC \cite{baele2017adaptive,suchard2018bayesian}. Baele et al. 2017  observed a large increase ($\approx 5-10 \times$) in sampling efficiency from using the AVMVN kernel on clock rates XXX(check)XXX and substitution model parameters across partitions \cite{baele2017adaptive}.  
...
Our experiments confirmed that $\text{NER}\{\mathcal{D}_{AE}, \mathcal{D}_{BE}, \mathcal{D}_{CE}\}$ is indeed superior on larger datasets (where $L > 1$kb; \textbf{Fig. \ref{fig:round4Results}}).
Whereas, on smaller datasets the standard \texttt{NarrowExchange} operator had a higher acceptance rate.
Using an adaptive operator (\texttt{AdaptiveOperatorSampler}) removes the burden from the user in selecting which operator is the best pick.
The average acceptance rate of 
...
Logistic and linear regression models are shown, respectively. }

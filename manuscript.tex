...
\begin{tabular}{|l| l l l|} 
 ...
 %3 & \textbf{Uniform operator} & $\begin{array} {rl} y &\leftarrow s\Sigma \times \frac{u - x}{x - l} \\ x^\prime &\leftarrow \frac{u + l * y}{y + 1}  \end{array}$ & $t, q$  \\
...
for random walk step size $\Sigma$ drawn from a uniform or \textbf{\nameref{sect:randomwalks}}. NER is compatible with both the \textit{real} and \textit{quant} parameterisations. Analagous to the Constant Distance operator, these rates are proposed such that genetic distances between nodes $A$, $B$, $C$, and $E$ are maintained. Thus, there are $\binom{4}{2} = 6$ pairwise distance constraints.

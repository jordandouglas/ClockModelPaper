...
This assumption forms the basis of phylogenetics, under which the evolutionary trees and divergence dates of life forms are inferred from biological sequences, such as nucleotides and amino acids \cite{douzery2003local, drummond2006relaxed}.
...
The domain of $\vec{\mathcal{R}}^{\,}$ is uniformly distributed under thr prior:
...
Let $f(x|\sigma)$ be the probability density function (PDF) and let $F(x|\sigma) = \int\limits_{0}^{x} f(t|\sigma) \; dt$ be the cumulative distribution function (CDF) of the prior distribution used by the underlying \textit{real} clock model. 
...
These operators are presented in \textbf{\nameref{sect:S1Appendix}}. 

The computational requirements of i-CDF evaluation may hinder the performance of the \textit{quant} parameterisation.
...
\texttt{Uniform} & Resamples one element in the vector from a uniform distribution. & $\vec{\mathcal{R}}^{\,}$  & \textit{cat}, \textit{quant}  \\
 \hline
\texttt{ConstantDistance} & Adjusts an internal node and recalculates all incident branch rates such that the genetic distances remain constant \cite{zhang2020improving}.  & $\vec{\mathcal{R}}^{\,}, \mathcal{T}$ & \textit{real}, \textit{quant} \\
 \hline
...
\texttt{SmallPulley} & Proposes new branch rates incident to the root such that their combined genetic distance is constant  \cite{zhang2020improving}.  & $\vec{\mathcal{R}}^{\,}$ & \textit{real}, \textit{quant} \\
 \hline
\texttt{FastClockScale} & Applies \texttt{Scale} to $\sigma$ and then recomputes all rates $\vec{\mathcal{R}}^{\,}$ such that their quantiles under the prior $p(\vec{\mathcal{R}}^{\,}|\sigma)$ are constant \cite{zhang2020improving}.  & $\mathcal{R}$ & \textit{real} \\
 \hline
\end{tabular}
...
\texttt{AdaptiveOperatorSampler} should learn the combination of weights behind these classes of operators best suited for any given dataset.
...
In the low signal dataset ($L=0.1$kb), both operators can traverse the likelihood space effectively.
 However, the same exact same proposal by \texttt{RandomWalk} incurs a much larger likelihood penalty in the $L=0.5$kb dataset by ``falling off the ridge'', in contrast to \texttt{ConstantDistance} which ``walks along the ridge''.
...
In this article we have introduced four new clock model operators. 
...
Therefore a weight of 10 corresponds to sampling probability of 0.086, for example.}

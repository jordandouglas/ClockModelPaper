...
% process. Please follow the template instructionssum 
% whenever possible.
%
% % % % % % % % % % % % % % % % % % % % % % %
%
...
In BEAST2, operators can have their own tunable parameter $s$ which determines the step size of the operator, and this term is learned over the course of the MCMC \cite{bouckaert2019beast}. 
...
The remaining terms are trained during the second and third phases: the cumulative computational runtime spent on each operator $\mathbb{T}(\omega_i)$, the sample standard deviation $\sigma_p$ of each numerical parameter $p$, and the summed distances $\sum_x \mathbb{D}(x_p, x_p^\prime)$.
...
Datasets with strong signal (or large $L$) are likely to achieve faster convergence when the more precise and meticulous kinds of operators are employed, such as those informed by correlations within the posterior distribution (\textbf{Fig \ref{fig:landscape}}).
...
We hypothesised that if \texttt{NarrowExchange} was adapted to the relaxed clock model by ensuring that genetic distances remain constant after the proposal (akin to the constant distance operators \cite{zhang2020improving}), then its ability to traverse the state space may improve. 
...
\paragraph{3. Automated generation of BEAST2 operators.} Java class files are generated using string processing. Each class corresponds to a single operator, extends the class of a meta-NER-operator, and consists of the solutions found in \textbf{1} and the Jacobian determinant found in \textbf{2}. $|J|$ is further augmented if the \textit{quant} parameterisation is employed (\textbf{\nameref{sect:S1Appendix}}).
...
Operators which apply to either internal nodes or the root (but not both) are weighted according to leaf count $N$. The adaptive operators are further broken down in \textbf{Tables \ref{table:adaptiveSampling}, \ref{table:adaptiveNER}, and \ref{table:AVMVNoperators}}.
...
In the most extreme case, leaf substitution rates $r$ and clock standard deviation $\sigma$ both mixed around 50 $\times$ faster on the 15.5 kb seed plant dataset \cite{Ran_2018} for \textit{real} than they did for \textit{cat}.
...
We wanted to test the applicability of the adaptive AVMVN kernel to leaf rate proposals.
This would operate by exploiting any correlations which occur between leaf branch substitution rates.
...
These results showed that the AVMVN operator yielded slightly better mixing (around 6\% faster) for the tree likelihood, the tree length and the mean branch rate (\textbf{Table \ref{fig:round5Results}}).
...
For the largest dataset considered (Ran et al. 2018 \cite{Ran_2018}), the new settings were up to 66 and 37 times as fast respectively.
...
These methodologies were compared by constructing phylogenetic models from several empirical datasets (\textbf{Fig \ref{fig:parameterisationResults}}) and comparing their abilities to converge in a tournament-like protocol (\textbf{Fig \ref{fig:tournament}}).
This work has produced an operator configuration which explores relaxed clock model space up to an order of magnitude more efficiently than previous setups.
...
We also found that Bactrian proposal kernels quite reliably increased mixing efficiency by 15--20\% (\textbf{Fig \ref{fig:round3Results}}).
Similar observations were made by Yang et al. \cite{yang2013searching}.
While this may only be a modest improvement, incorporation of the Bactrian kernel into pre-existing operators is a computationally straightforward task and should be routinely implemented in Bayesian MCMC software packages.
...
Lakner et al. categorised tree operators into two classes.
...
Lakner et al. observed that topological proposals made by the former class consistently outperformed topological changes invoked by the latter class \cite{lakner2008efficiency}.
...
This topological change in the landscape necessitates the use of operators which exploit known correlations in the posterior density, such as AVMVN \cite{baele2017adaptive}, constant distance operators \cite{zhang2020improving}, and the narrow exchange rate operators introduced in this article.
...
For instance, constant distance operators outperformed standard operator configurations by up to an order of magnitude on larger datasets but they were up to three times slower on smaller ones (\textbf{Fig \ref{fig:round1Results}}). 
...
Given the immense availability of sequence data, and the increased computational cost required to process it, high performance on large datasets is a desirable property.

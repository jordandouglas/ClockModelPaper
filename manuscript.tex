...
Statistical inference is typically performed by sampling using the Markov chain Monte Carlo (MCMC) algorithm \cite{metropolis53, hastings70}.
...
The tree likelihood $p(D|\mathcal{T}, r(\vec{\mathcal{R}}^{\,}), \theta)$ is computed using the tree-peeling algorithm \cite{felsenstein1981evolutionary}, where
...
In a relaxed clock model, each of the $2N-2$ elements in $\mathcal{R}_i$ are independently distributed under the prior $p(\vec{\mathcal{R}}^{\,} | \sigma)$.
...
\caption{\textbf{Branch rate parameterisations.} Top left: the branch rate prior under a log-normal($-0.5\sigma^2, \sigma$) distribution (with a mean of 1). 
...
We hypothesise that datasets with strong signal (or large $L$) will mix best when the more precise and meticulous kind of operator is employed, such as those informed by correlations within the posterior distribution e.g. the family of constant distance operators \cite{zhang2020improving}.
...
Each methodology is benchmarked across 5 replicates on each dataset, using an the Intel Xeon Gold 6138 CPU (2.00 GHz) and all methodologies use identical models and operator configurations, except where a difference is specified.

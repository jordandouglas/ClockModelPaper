...
This work has produced an operator scheme that is up to 65 times more efficient at exploring continuous relaxed clock parameters compared with previous setups, depending on the dataset. %RB: 65 or 66 mentioned later?
...
This work has produced a phylogenetic setup which can infer parameters related to the relaxed clock up to 65 times faster than previous setups, depending on the dataset. %RB: 65 or 66 mentioned later?
...
In the remainder of this paper %RB: "article" and "paper" are used a few times -- should we keep things consistent to either "paper" or "article"?
...
\textul{\textit{Step 2}}. Identify the child of $E$ with the greater height. Denote this child as $D$ and its sibling as $C$ (i.e. $t_D > t_C$).
 %RB: C must not be internal, right? If C is leaf, abort?
...
In all phylogenetic analyses presented here, we use a Yule \cite{yule1925ii} tree prior $p(\mathcal{T}|\lambda)$ with birth rate $\lambda \sim \text{Log-Normal}(1,1.25)$. %RB: mean in real space? This is not the traditional parameterisation in the literature, and it was also mentioned earlier that the mean parameter is set such that the mean=1
...
Overall, this operator configuration is the final winner in the tournament. %RB: which configuration are you referring to?
...
The methods introduced are adaptive and treat each dataset differently, and perform never worse than without adaptation. %RB: is the part after the comma true?
